...
//			btVector3 rel_pos2 = pos2 - colObj1->getWorldTransform().getOrigin();
			...
			rel_vel = cp.m_normalWorldOnB.dot(vel);
				...
				if (infoGlobal.m_solverMode & SOLVER_USE_WARMSTARTING)
				{
					...
					if (rb1)
						rb1->internalApplyImpulse(solverConstraint.m_contactNormal*rb1->getInvMass()*rb1->getLinearFactor(),-solverConstraint.m_angularComponentB,0-solverConstraint.m_appliedImpulse);
				} else
				{
					solverConstraint.m_appliedImpulse = 0.f;
				}
					...
					{
						{
							...
							if (infoGlobal.m_solverMode & SOLVER_USE_WARMSTARTING)
							{
								...
								if (rb1)
									rb1->internalApplyImpulse(frictionConstraint1.m_contactNormal*rb1->getInvMass()*rb1->getLinearFactor(),-frictionConstraint1.m_angularComponentB,0-frictionConstraint1.m_appliedImpulse);
							} else
							{
						...
						if ((infoGlobal.m_solverMode & SOLVER_USE_2_FRICTION_DIRECTIONS))
						{
							...
							if (infoGlobal.m_solverMode & SOLVER_USE_WARMSTARTING)
							{
								...
								if (rb1)
									rb1->internalApplyImpulse(frictionConstraint2.m_contactNormal*rb1->getInvMass(),-frictionConstraint2.m_angularComponentB,0-frictionConstraint2.m_appliedImpulse);
							} else
							{
